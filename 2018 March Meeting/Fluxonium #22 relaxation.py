...
for Q_cap in [0.3e6]:
    ...
    plt.semilogy(phi_ext, 1.0/gamma_cap *1e6, linewidth= 2.0, linestyle ='-', color = 'blue')

for x_qp in [5e-7]:
    Q_qp = 1.0/x_qp
    for idx in range(len(phi_ext)):
        gamma_qp[idx] = r_qp(E_l, E_c, E_j, Q_qp, w[idx], qp_element[idx])
    plt.semilogy(phi_ext, 1.0/(gamma_qp)*1e6, linewidth = 2.5, linestyle='--', color = 'red')
...
#     plt.semilogy(phi_ext, 1.0/(gamma_qp+gamma_cap+gamma_qp_array)*1e6, linewidth = 2.5, linestyle='-')
