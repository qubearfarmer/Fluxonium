...
ax2 = ax1.twinx()
ax2. plot(phi_ext, energy, linewidth = 2.0, color = 'k')
plt.tick_params(labelsize = 18.0)
# ax2.plot(phi_ext,abs(sensitivity), '--', )
ax1.semilogy(phi_ext,(1.8e-6*2*np.pi*abs(sensitivity)*1e9*np.sqrt(np.log(2)))**-1 *1e6, '--', )
...
T2 = data[:,1]
T2_error = data[:,2]
ax1.errorbar((current-59.585)/1.53 * 0.5, T2, yerr = T2_error, fmt='h', mfc='none', mew=2.0, mec='r', ecolor='r')
