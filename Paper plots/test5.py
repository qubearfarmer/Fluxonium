...
phase = - phase
phase = phase - np.mean(phase)
plt.plot(time/1e3, phase, 'd', mfc='none', mew='2', mec='red')
...
plt.plot(time_nice / 1e3, func(time_nice * 1e-9, a, b, c, d, g), linewidth='2', color='k')

plt.tick_params(labelsize=18)
# plt.xlim([0,np.max(time_nice/1e3)])
plt.xlim([0,5])
plt.ylim([-0.8,0.8])
# plt.show()
'''
...
print a
time_nice = np.linspace(0, np.max(time), 1000)
plt.plot(time / 1e3, phase - guessA / 2, 'd', mfc='none', mew='2', mec='red')
plt.plot(time_nice / 1e3, T1_func(time_nice * 1e-9, a, b, c, d) - guessA / 2, linewidth='2', color='k')
...
offset = t2_fit[-1]
plt.plot(time/1e3,phase - offset, 's', mfc='none', mew='2', mec='g')
plt.plot(time_nice / 1e3, t2_fit - offset, linewidth='2', color='k')
