...
for idx in range(len(T1)):
    if (energies2[idx, 2] - energies2[idx, 1]) > 2.5 and (energies2[idx, 2] - energies2[idx, 1]) < 3.5:
...
# plt.errorbar(p_element21 ** 2, T1_final2, yerr=T1_err_final2, fmt='d', mfc='none', mew='2', mec='red',ecolor = 'r')
# for Q_cap in [7e4, 7e5]:
#     for idx in range(len(pem)):
#         gamma_cap[idx] = r_cap(E_l, E_c, E_j_sum, d, Q_cap, w, pem[idx])
#     plt.loglog(pem**2, 1.0 / (gamma_cap) * 1e6, linewidth='2', linestyle ='--', color = 'k')

#QP loss
plt.errorbar(qp_element[:, 0] ** 2 + qp_element[:, 1] ** 2, T1_final, yerr=T1_err_final, fmt='s', mfc='none', mew='2', mec='blue')
plt.errorbar(qp_element21[:, 0] ** 2 + qp_element21[:, 1] ** 2, T1_final2, yerr=T1_err_final2, fmt='d', mfc='none', mew='2', mec='red',ecolor = 'r')
for Q_qp in [5e5, 5e6]:
    for idx in range(len(qpem)):
        gamma_qp[idx,:] = r_qp(E_l, E_c, E_j_sum, d, Q_qp, w, [qpem[idx],qpem[idx]])
    plt.loglog((qpem)**2*2, 1.0 / (gamma_qp[:,0]+gamma_qp[:,1]) * 1e6, linewidth='2', linestyle ='--', color = 'k')
