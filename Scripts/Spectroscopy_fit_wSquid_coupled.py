...
iState = 0
fState = 1
guess = ([E_l_guess, E_c_guess, E_j_sum_guess, A_j_guess, A_c_guess, d_guess, offset_squid_guess, offset_ext_guess, g_guess])
def fit_func(current, E_l, E_c, E_j_sum, d, A_j, A_c, offset_squid, offset_ext, g):
    ...
    phi_squid = 2 * np.pi * (flux_squid / phi_o - offset_squid)
    phi_ext = 2 * np.pi * (flux_ext / phi_o - offset_ext)
...
opt, cov = curve_fit(fit_func, flux_points, freq_points, guess)

#####################################################################################
#####################################################################################
E_l_fit, E_c_fit, E_j_fit, A_j_fit, A_c_fit, d_fit, offset_squid_fit, offset_ext_fit, g_fit = opt
...
current_nice = np.linspace(0.02, 0.03,101)
spec_fit = fit_func(current_nice, E_l_fit, E_c_fit, E_j_fit, d_fit, A_j_fit, A_c_fit, offset_squid_fit, offset_ext_fit, g_fit)
plt.plot(current_nice*1e3, spec_fit)

E_l_fit, E_c_fit, E_j_fit, A_j_fit, A_c_fit, d_fit, offset_squid_fit, offset_ext_fit, g_fit = guess
spec_fit = fit_func(current_nice, E_l_fit, E_c_fit, E_j_fit, d_fit, A_j_fit, A_c_fit, offset_squid_fit, offset_ext_fit, g_fit)
plt.plot(current_nice*1e3, spec_fit)
