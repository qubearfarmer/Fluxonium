...
gamma_qp_array = np.zeros(len(current))
gamma_qp = np.zeros((len(current),2))

# for Q_cap in [0.8e6]:
#     for idx in range(len(current)):
#         gamma_cap[idx] = r_cap(E_l, E_c, E_j_sum, d, Q_cap, w[idx], p_element[idx], T_diel)*thermal_factor_diel[idx]
...
for x_qp in [3e-9]:
    ...
    plt.semilogy(current, 1.0 / (gamma_qp[:,0]*thermal_factor_qp[idx] + gamma_qp[:,1]*thermal_factor_qp[idx]) * 1e6, linewidth='2', linestyle ='-')
#

# for x_qp in [12e-9]:
#     Q_qp = 1.0 / x_qp
#     for idx in range(len(current)):
#         gamma_qp_array[idx] = r_qp_array(E_l, E_c, E_j_sum, Q_qp, w[idx], p_element[idx])*thermal_factor_qp[idx]
#     plt.semilogy(current, 1.0 / (gamma_qp_array) * 1e6, linewidth=2.0, linestyle='--', color = 'orange')
